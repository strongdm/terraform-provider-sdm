...
func TestAccSDMAccount_UserCreate(t *testing.T) {
	...
	accounts, err := createUsersWithPrefix("manager-account", 1)
	managerID := accounts[0].GetID()
	if err != nil {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSDMAccountUserPermissionLevelAndManagerIDConfig(rsName, firstName, lastName, email, permissionLevel, managerID),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("sdm_account."+rsName, "user.0.manager_id", managerID),
...
func TestAccSDMAccount_Update(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccSDMAccountUserPermissionLevelAndManagerIDConfig(rsName, firstName2, lastName2, email2, permissionLevel2, managerID),
				Check: resource.ComposeTestCheckFunc(
...
func testAccSDMAccountUserPermissionLevelAndManagerIDConfig(resourceName, firstName, lastName, email, permissionLevel, managerID string) string {
	return fmt.Sprintf(`
	resource "sdm_account" "%s" {
		user {
			first_name = "%s"
	...
	`, resourceName, firstName, lastName, email, permissionLevel, managerID)
}
...
func createUserWithManagerID(prefix string, managerID string) (sdm.Account, error) {
	client, err := preTestClient()
	if err != nil {
		return nil, fmt.Errorf("failed to create test client: %w", err)
	}
