...
func TestAccSDMResource_CreatePortOverride(t *testing.T) {
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						func(s *terraform.State) error {
							...
							redis := resp.Resource.(*sdm.Redis)
							if redis.Name != name {
								return fmt.Errorf("unexpected name '%s', expected '%s'", redis.Name, name)
							}
							if redis.PortOverride != portOverride {
								return fmt.Errorf("unexpected port override '%v', expected '%v'", redis.PortOverride, portOverride)
							}
			...
			Steps: []resource.TestStep{
				{
					...
					Check: resource.ComposeTestCheckFunc(
						...
						func(s *terraform.State) error {
							...
							if redis.PortOverride == 0 {
								return fmt.Errorf("missing calculated port override")
							}
